...
public class UpdateUiToController : MonoBehaviour {
	...
	void Update () {
        JInput.Controller controller = JInput.Input.m_Input.getController(m_ControllerIndex);
        if(controller == null) {
            return;
        }
        ...
        m_PovText.text = JInput.Input.GetNameFromHatDir(controller);
		...
		for(int i = 0; i < controller.data.buttons.Length; i++) {
			if (controller.data.buttons[i]) {
				...
				buttons++;
				buttonsDown += JInput.Input.GetNameFromButton(controller, i);
			}
		}
